...
void ObstacleDetection::saveResult(){
  ...
  std::string obs_single_path;
  std::string obs_multi_path;
  std::stringstream ss;
  ...
  for(int i = 0; i<row; i++){
    for(int j = 0; j<col; j++){
      double distance = (i-center_row)*(i-center_row)+(j-center_col)*(j-center_col);
      distance = 0.2*sqrt(distance);
      int index = (int)distance;
      if(index >= 0 && index <= 100){
        if(_grid_attr_single[i][j].attribute != UNKNOWN){
          single[index]++;
        }
        if(_grid_attr_multi[i][j].attribute != UNKNOWN){
          multi[index]++;
        }
      }
    }
  ...
  std::ofstream f_single(obs_single_path, std::ofstream::app);
  for(int i = 0; i < 100; i++){
    f_single<<i<<"  "<<single[i]<<std::endl;
  }
  f_single.close();

  std::ofstream f_multi(obs_multi_path, std::ofstream::app);
  for(int i = 0; i < 100; i++){
    f_multi<<i<<"  "<<multi[i]<<std::endl;
  }
  f_multi.close();
